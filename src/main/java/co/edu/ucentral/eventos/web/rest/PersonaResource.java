...
import co.edu.ucentral.eventos.repository.PersonaRepository;
...
/**
 * REST controller for managing {@link co.edu.ucentral.eventos.domain.Persona}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PersonaResource {
     ...
     */
    @GetMapping("/personas")
    public List<Persona> getAllPersonas(@RequestParam(required = false) String filter) {
        if ("estudiante-is-null".equals(filter)) {
            log.debug("REST request to get all Personas where estudiante is null");
            return StreamSupport
                .stream(personaRepository.findAll().spliterator(), false)
                .filter(persona -> persona.getEstudiante() == null)
                .collect(Collectors.toList());
        }
        if ("usuarioucentral-is-null".equals(filter)) {
            log.debug("REST request to get all Personas where usuarioUcentral is null");
            return StreamSupport
                .stream(personaRepository.findAll().spliterator(), false)
                .filter(persona -> persona.getUsuarioUcentral() == null)
                .collect(Collectors.toList());
        }
        if ("profesor-is-null".equals(filter)) {
            log.debug("REST request to get all Personas where profesor is null");
            return StreamSupport
                .stream(personaRepository.findAll().spliterator(), false)
                .filter(persona -> persona.getProfesor() == null)
                .collect(Collectors.toList());
        }
        if ("delegadoinstitucional-is-null".equals(filter)) {
            log.debug("REST request to get all Personas where delegadoInstitucional is null");
            return StreamSupport
                .stream(personaRepository.findAll().spliterator(), false)
                .filter(persona -> persona.getDelegadoInstitucional() == null)
                .collect(Collectors.toList());
        }
        if ("evaluador-is-null".equals(filter)) {
            log.debug("REST request to get all Personas where evaluador is null");
            return StreamSupport
                .stream(personaRepository.findAll().spliterator(), false)
                .filter(persona -> persona.getEvaluador() == null)
                .collect(Collectors.toList());
        }
